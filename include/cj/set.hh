#ifndef CJ_MATH_SET_HH_
#define CJ_MATH_SET_HH_
...
namespace cj {
   ...
   */
  template<typename T>
  auto set_union_size(boost::container::flat_set<T> const& xs,
    ...
    while (xs_it++ != xs_end) ++count;
    while (ys_it++ != ys_end) ++count;
    return count;
  }
   ...
   */
  template<typename T>
  auto set_union_size(std::set<T> const& xs,
                      std::set<T> const& ys)
                      noexcept -> size_t {
    size_t count = 0;
    auto const xs_end = xs.end(), ys_end = ys.end();
   ...
   */
  template<typename T>
  auto set_union_size(std::unordered_set<T> const& xs,
