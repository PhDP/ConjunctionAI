...
#include "cj/utils/clreader.hh"
#include "cj/learning/mutate_fuzzy_kb.hh"
#include "cj/fuzzy_classifier.hh"
#include "cj/math/statistics.hh"

auto main(int argc, char *argv[]) -> int {
  auto const verbosity = cj::get_arg<uint32_t>(argc, argv, "verbosity", 0);
  ...
  auto const elites = std::min(cj::get_arg<uint32_t>(argc, argv, "elites", pop_size / 4), pop_size);
  auto const nonelites = pop_size - elites;
  auto const mutations = cj::get_arg<uint32_t>(argc, argv, "mutations", 1000);
  auto const crossovers = cj::get_arg<uint32_t>(argc, argv, "crossovers", 10);
  auto const alpha = cj::get_arg<double>(argc, argv, "alpha", 0.0005);
  auto const ptest = 0.1;

  auto logic = cj::get_lukasiewicz_logic<double>();
  ...
  if (logic_name != "Łukasiewicz") {
    if (logic_name == "Godel" || logic_name == "Gödel" || logic_name != "Gödel-Dummett") {
      ...
      logic = cj::get_godel_logic<double>();
    } else if (logic_name == "Product") {
      logic = cj::get_product_logic<double>();
    } else {
  ...
  std::cout
    ...
    << "  Mutations per crossover: " << mutations << '\n'
    << "  Complexity penalty (alpha): " << alpha << '\n'
    << "  Proportion held for testing: " << ptest << '\n';

  auto data_ref = cj::datamatrix_from_file<double>("../data/poll_plant/poll.csv");
  if (!data_ref) {
  ...
  std::cout << "\nInput variables:\n";
  for (auto h = 0u; h < headers.size() - 1; ++h) {
    std::cout << "  " << h << ": " << headers[h] << '\n';
  }

  std::cout << "\nEntries in the data-set: " << data.nrows() << '\n';

  auto test = data.split_frame(ptest, seed);
